...
//══════════[ FITUR FITUR OY ]══════════//
case 'menu':
gambar = fs.readFileSync('./media/dhani.jpg')
totalChat = await Dhani.chats.all()
groups = Dhani.chats.array.filter(v => v.jid.endsWith('g.us'))
privat = Dhani.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
timestampe = speed();
latensie = speed() - timestampe
menu = `${botname}

「 𝙄𝙉𝙁𝙊 𝘽𝙊𝙏 」
➢ 𝘕𝘢𝘮𝘢 𝘰𝘸𝘯𝘦𝘳 : ${ownername}
➢ 𝘕𝘢𝘮𝘢 𝘣𝘰𝘵 : ${botname}
➢ 𝘛𝘰𝘵𝘢𝘭 𝘎𝘳𝘰𝘶𝘱 : ${groups.length}
➢ 𝘛𝘰𝘵𝘢𝘭 𝘗𝘳𝘪𝘣𝘢𝘥𝘪 : ${privat.length}
➢ 𝘛𝘰𝘵𝘢𝘭 𝘊𝘩𝘢𝘵 : ${totalChat.length}
➢ 𝘙𝘶𝘯𝘵𝘪𝘮𝘦 : ${runtime(process.uptime())}
➢ 𝘚𝘱𝘦𝘦𝘥 : ${latensie.toFixed(4)} _Second_

「 𝙄𝙉𝙁𝙊 𝙐𝙎𝙀𝙍 」
➢ 𝘕𝘢𝘮𝘢 : ${pushname}
➢ 𝘕𝘰𝘮𝘰𝘳 : wa.me/${sender.split('@')[0]}
➢ 𝘚𝘵𝘢𝘵𝘶𝘴 : ${isOwner ? 'Owner' : 'User'}
...
➢ 𝘞𝘪𝘵𝘢 : ${timeMak}
➢ 𝘞𝘪𝘵 : ${timeJay}`
teks = `*_© ${ownername} || 2022_*`
but = [
{ buttonId: `${prefix}allmenu`, buttonText: { displayText: '𝘼𝙇𝙇𝙈𝙀𝙉𝙐' }, type: 1 },
{ buttonId: `${prefix}sewabot`, buttonText: { displayText: '𝙎𝙀𝙒𝘼𝘽𝙊𝙏' }, type: 1 }
]
sendButImage(from, menu, teks, gambar, but)
break

case 'allmenu':
gambar = fs.readFileSync('./media/kntl.jpg')
menu =
...
┃• _Speed :_ ${latensie.toFixed(4)} _Second_
┗━━⬣
...
▢ BISA BIKIN STIKER TEKS
▢ BISA BIKIN STIKER VIDIO 
▢ BISA BIKIN STIKER GAMBAR
...
𝐌𝐚𝐬𝐮𝐤 𝐆𝐫𝐮𝐛 - 𝐓𝐫𝐚𝐧𝐬𝐟𝐞𝐫 - 𝐁𝐨𝐭 𝐉𝐚𝐝𝐢𝐢𝐧 𝐀𝐝𝐦𝐢𝐧 - 𝐀𝐤𝐭𝐢𝐟𝐢𝐧 𝐅𝐢𝐭𝐮𝐫 - 𝐃𝐨𝐧𝐞

💸𝘗𝘈𝘠𝘔𝘌𝘕𝘛 :
𝘋𝘈𝘕𝘈 : 089665569934 𝘢/𝘯 𝘑𝘢𝘴𝘯𝘪
𝘘𝘙𝘐𝘚 𝘈𝘓𝘓 𝘗𝘈𝘠𝘔𝘌𝘕𝘛`
teks =
`*『 ${botname} 』*
*${tanggal}*`
Dhani.sendMessage(from, { contentText: `${menu}`, footerText: `${teks}`, buttons: [{ buttonId: `${prefix}scan`, buttonText: { displayText: '𝙌𝙍𝙄𝙎 𝘼𝙇𝙇 𝙋𝘼𝙔𝙈𝙀𝙉𝙏' }, type: 1 },{ buttonId: `${prefix}owner`, buttonText: { displayText: '𝙊𝙒𝙉𝙀𝙍' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: fakeimage, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
break

case 'scan':
gambar = fs.readFileSync('./media/scan.jpg')
menu = `SCAN KAK QRIS ALL PAYMENT BISA BUAT TF YANG BELUM PREMIUM`
teks = `*_Powered By ${botname}_*`
but = [
      { buttonId: `${prefix}menu`, buttonText: 
{ displayText: '𝙈𝙀𝙉𝙐' }, type: 1 },
    ]
    sendButImage(from, menu, teks, gambar, but)
break
...
menu = `*Hi Kak ${pushname} Mau Donasi? Pilih Payment Di Bawah Ya :)*
𝑷𝑨𝒀𝑴𝑬𝑵𝑻 : 
𝐷𝑎𝑛𝑎 : 089665569934 𝑎/𝑛 𝐽𝐴𝑆𝑁𝐼
𝑄𝑟𝑖𝑠 𝐴𝑙𝑙 𝑃𝑎𝑦𝑚𝑒𝑛𝑡 : 𝑘𝑒𝑡𝑖𝑘 𝑏𝑢𝑡𝑜𝑛 𝑑𝑖 𝑏𝑎𝑤𝑎ℎ`

teks =
`*_© ${ownername} || 2022_*`
but = [
      { buttonId: `${prefix}scan`, buttonText: 
{ displayText: '𝙌𝙍𝙄𝙎 𝘼𝙇𝙇 𝙋𝘼𝙔𝙈𝙀𝙉𝙏' }, type: 1 },
    ]
    sendButLocation(from, menu, teks, gambar, but)
break

case 'script':
menu = `*Info Script* 🔎\n📚 Script Made By Tama Botz\n💾 Version : 1.0.0\n📈 Last Update : 11/2/2022\n📩 Link Sc : Hubungi Owner Yo Gays wa.me/6282314348080 (Whexz Ganz) Sumber Script : Tama Botz\n\n🍁 Subscribe For More Update`
teks =`${botname} | Runtime : ${runtime(process.uptime())}\nOwner : ${ownername}`
Dhani.sendMessage(from, { contentText: `${menu}`, footerText: `${teks}`, buttons: [{ buttonId: `${prefix}donasi`, buttonText: { displayText: '𝘿𝙊𝙉𝘼𝙎𝙄' }, type: 1 },{ buttonId: `${prefix}owner`, buttonText: { displayText: '𝙊𝙒𝙉𝙀𝙍 𝘽𝙊𝙏𝙕' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: fakeimage, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
break
case 'rulesbot':
...
: mek;
owgi = await Aikko.downloadMediaMessage(boij);
...
case 'done':
menu =`DONE TQ DAH ORDER
*⚡ BUYYER : ${pushname}*
*📱 NOMOR :* wa.me/${sender.split('@')[0]}
*⏰ JAM :  ${time} WIB*
*🗓️ TANGGAL :  ${tanggal}*

all trx no reff kalau dah bilang done jadi dah ga aada hubungan transaksi lagi antara kedua belah pihak`
reply(menu)
break

case 'p':
case 'proses':
menu = `ORDERAN KAMU SEDANG DI  PROSES
*⏰ JAM ⵓ ${time} WIB*
*🗓️ TANGGAL ⵓ ${tanggal}*

𝇅𝇃𝇅 ᣟ𝘁𝗵𝗲 𝗽𝗿𝗼𝗰𝗲𝘀𝘀 𝘁𝗮𝗸𝗲𝘀 𝗮𝗯𝗼𝘂𝘁 1-𝟭𝗢 𝗺𝗶𝗻𝘂𝘁𝗲𝘀̡ 𝗲𝘃𝗲𝗿𝘆𝘁𝗵𝗶𝗻𝗴 𝗰𝗼𝗺𝗲𝘀 𝗯𝗮𝗰𝗸 𝘁𝗼 𝘆𝗼𝘂 𝗶𝗳 𝘆𝗼𝘂 𝗮𝗿𝗲 𝗳𝗮𝘀𝘁 𝗼𝘄𝗻𝗲𝗿𝘀 𝘁𝗼𝗼 𝗳𝗮𝘀𝘁`
reply(menu)
break
...
fakeyt(`_Tuh Kak Ownerku Jangan Jahil Ya!!_`)
break
case 'bc':
